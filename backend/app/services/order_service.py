...
async def send_order_confirmation_email_background(order_id: int):
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            order = result.scalar_one_or_none()
            if not order:
                print(f"WARNING: Order #{order_id} not found in database for background email.")
                return
            
            customer = order.customer
            if not customer or not customer.email:
                print(f"WARNING: Order #{order.id} has no customer email or customer is None. Skipping email.")
                return
